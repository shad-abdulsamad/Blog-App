    ...
    <div class="container py-md-5 container--narrow">
      <h2>
        ...
        @if(!$currentlyFollowing AND auth()->user()->username != $username)
              <form class="ml-2 d-inline" action="/create-follow/{{$username}}" method="POST" enctype="multipart/form-data">
          @csrf
          <button class="btn btn-primary btn-sm">Follow <i class="fas fa-user-plus"></i></button>
        </form>
        @endif
        @if($currentlyFollowing)
              <form class="ml-2 d-inline" action="/remove-follow/{{$username}}" method="POST" enctype="multipart/form-data">
          @csrf
     ...
     @if (auth()->user()->username == $username)
      ...
      <div class="profile-nav nav nav-tabs pt-2 mb-4">
        <a href="/profile/{{$username}}" class="profile-nav-link nav-item nav-link active">Posts: {{$postCount}}</a>
